...
bool SPI_Flash_WriteV(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)   
{ 
	u32 secpos;
	u16 secoff;
	u16 secremain;	   
	...
	SPI_Flash_Read(SPI_FLASH_BUF, addr, writelen);
	for(i=0; i<writelen; i++)
	{
		if(pBuffer[i]!=SPI_FLASH_BUF[i])
			return FALSE;	
	}
	return TRUE;	 	 
}
...
////不能做随机读写，必须要按顺序写，并且每次写的字节数可以被4096整除  
//此函数被修改，fan  后面加了读校验功能	校验最多256个字节
void SPI_Flash_WriteQ(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite) 
{ 
	u32 secpos;
	...
	//如果从扇区第一个字节开始，擦除扇区	////
	if( (WriteAddr & (u32)0x0FFF)==0 )
	{
//		SPI_Flash_Read(SPI_FLASH_BUF,secpos*4096,4096);	//读出整个扇区的内容
		SPI_Flash_Erase_Sector(secpos);					//擦除这个扇区
	}

	SPI_Flash_Write_NoCheck(pBuffer,WriteAddr,NumByteToWrite);	//直接写入扇区区间
}
//#endif
//擦除整个芯片
//整片擦除时间:
...
bool SPI_Flash_Init(void)
{
	...
	//如果该数据区块内有数据将会被删除(因此最后一个扇区必须要留出来)
	for(t=0;t<2;t++)
	{
	  	SPI_Flash_Write(&id0[t], addr, 1);
	    SPI_Flash_Read(&id1[t], addr, 1);
		if(id0[t]!=id1[t])
		return FALSE;
	}
    return TRUE;
